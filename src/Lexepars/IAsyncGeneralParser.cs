...
{
    /// <summary>
    /// Parses a stream of tokens asynchronously providing a <see cref="IGeneralReply"/> indicating success of failure as the result. There is no returned value with the reply, so this is suitable for validation situations or for the cases when the returned value is not needed because is gets skipped.
    /// </summary>
    public interface IAsyncGeneralParser
    {
        /// <summary>
        ...
        Task<IGeneralReply> ParseGenerallyAsync(TokenStream tokens, CancellationToken cancellationToken);
